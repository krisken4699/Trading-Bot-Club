...
function Navbar() {
  ...
  return (
    <Nav className={`z-50 fixed px-10 ${top ? "text-secondary" : "bg-[#f7f7f7]"} hover:opacity-90 transition-all duration-500`}>
      <div>
        <div className="grid lg:px-36 grid-cols-3 grid-rows-1 gap-x-0 ">
          <div className="nav-collapse hidden lg:col-start-1 dark:text-accent2 text- grid-rows-1 col-span-1 lg:grid grid-cols-3 gap-0 content-center text-center">
            ...
            <div className="a col-start-2 col-span-1 flex"><Link getProps={isActive} className="self-center text-center px-3 link" to="/dashboard/" href="#2">Dashboard</Link></div>
            <div className="a col-start-3 col-span-1 flex"><Link getProps={isActive} className="self-center text-center px-3 link" to="/page-3/" href="#3">Page 3</Link></div>
          </div>
          <div className='col-start2 col-span-1 flex items-center justify-center'>
            <Link to='/' className="link lg:hidden inline text-primary  dark:text-white !font-A1 text-md align-baseline font-semibold">TBC</Link>
            <Link to='/' className="link hidden lg:inline dark:text-white text-primary !font-A1 text-md align-baseline font-normal">Trading Bot Club</Link>
          </div>
          <div className="nav-collapse hidden lg:col-start-3 col-span-1 lg:grid grid-cols-1 gap-0 content-center text-center">
            <div className="col-start-3 flex">
              <a className="self-center text-center px-3 font-Metric-Medium tracking-1px text-[0.8em] dark:text-accent2 leading-5 not-italic tracking-[0.4px] font-light cursor-pointer uppercase text-333 hover:text-A29F9A font-500">{cookies.user ? "Logout" : "Sign in"}</a>
          ...
          <div ref={dropdownRef} id="dropdown-menu" className="max-h-screen overflow-y-hidden lg:inline origin-top-right right-0 mt-0 w-56 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
            <div className="py-1" role="none">
              ...
              <Link getProps={isActive} onClick={toggleDropdown} href="#2" className="link block px-4 py-2" role="menuitem" to="/dashboard/">Dashboard</Link>
              <Link getProps={isActive} onClick={toggleDropdown} href="#3" className="link block px-4 py-2" role="menuitem" to="/page-3/  ">License</Link>
              <a className="cursor-pointer self-center text-center py-2 px-4 font-Metric-Medium tracking-1px text-xs uppercase text-333 hover:text-A29F9A font-500">{cookies.user ? "Logout" : "Sign in"}</a>
            </div>
          </div>
